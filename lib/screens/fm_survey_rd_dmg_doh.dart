...
import 'package:mtracking/models/dmg_detail_model.dart';
...
class _SurveyRdDmgDohState extends State<SurveyRdDmgDoh> {
  ...
  Future<void> loadDmgDetail() async {
    if (listDmgDetailModel.length > 0) {
    ...
    if (response != null) {
      ...
      Map<String, dynamic> dmgDetList = resJs['RD_DAMAGE_DETAIL'];

      dmgDetList.forEach((k, v) {
        // print('$k: $v');

        DmgDetailModel dmgDetailModel = DmgDetailModel.fromJlist(k, v);
  ...
  Widget roadCodeForm() {
    return Container(
      width: 300.0,
      child: TextField(
        focusNode: _focusRdCode,
        onChanged: (String string) {
          rdCode = string.trim();
        },
        decoration: InputDecoration(hintText: 'รหัสสายทาง : '),
      ),
    );
  }
  ...
  Widget dmgDetailList() {
    return Container(
        ...
        child: DropdownButton(
            ...
            value: dmgDetSelected,
            onChanged: (DmgDetailModel newVal) {
              setState(() {
                dmgDetSelected = newVal;
              });
            }));
  }
